use std::{io::{ErrorKind, Read as _}, path::Path};
 ...
 */
pub fn read_samples<P: AsRef<Path>, const N: usize, const S: usize, const C: usize>(path: P) -> Result<impl Iterator<Item = [f32; N]>, ()> {
    ...
    let stdout = cmd.stdout(std::process::Stdio::piped())
        .spawn()
        .map_err(|_| ())?
        .stdout
        .ok_or(())?;
