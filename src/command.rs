...
pub async fn ffmpeg_duration<T: AsRef<Path>>(ffmpeg_home: Option<T>, input: T) -> Result<f32, CommandError> {
    ...
    if let Ok(_) = BufRead::read_line(&mut buf, &mut dur_str) {
        Ok(f32::from_str(&dur_str.trim()).map_err(|_| CommandError::ParsingError { context: "f32 parsing failed".into()})?)
    } else {
    ...
    let mut child = Command::new(&ffprobe_bin)
        ...
        loop {
            if AsyncBufReadExt::read_line(&mut reader, &mut buf).await? > 0 {
                keyframes.push(f32::from_str(buf.trim()).map_err(|_| CommandError::ParsingError { context: "f32 parsing failed".into()})?);
                buf.clear();
            } else {
                break;
            }
...
impl StreamRange {
    ...
    pub fn secs_betweens(pts_start: Option<f32>, pts_end: Option<f32>) -> Self {
        Self {
            pts_start,
            pts_end,
            skip_frames: Some("none"),
    ...
    pub fn minutes_betweens(pts_start: Option<f32>, pts_end: Option<f32>) -> Self {
        Self {
            pts_start,
            pts_end,
            skip_frames: Some("none"),
            frame_rate: Some(1.0/60.0),
        }
    }

    pub fn nsecs_betweens(pts_start: Option<f32>, pts_end: Option<f32>, secs: usize) -> Self {
        Self {
            pts_start,
            pts_end,
            skip_frames: Some("none"),
            frame_rate: Some(1.0/(secs as f32)),
        }
    }
}

pub async fn ffmpeg_extract_frames<T: AsRef<Path>>(ffmpeg_home: Option<T>, input: T, output: T, range: &StreamRange) -> Result<Vec<f32>, CommandError> {
    let ffmpeg_bin = ffmpeg_home.map(|p| p.as_ref().join("/bin/ffmpeg").display().to_string()).unwrap_or("ffmpeg".into());
