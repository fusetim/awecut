...
use error::{AppError, FingerprintError};
use pack::PackFile;
        ...
        for file in dir {
            if let Some(name) = file.file_name() {
                ...
                match calculate_fingerprint(&file, &conf).await {
                    Ok(fg) => {
                        pack.1.fingerprint.insert(index, (name, fg));
                    }
                    Err(err) => {
                        log::error!(
                ...
                log::error!("Ignoring {} (no filename).", file.display());
            }
            bar.inc(1);
        }
...
async fn calculate_fingerprint<T: AsRef<Path>>(
    path: T,
    config: &FpConf,
